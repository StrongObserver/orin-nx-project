...
def patch_reuse(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    if "VPI_EGLIMAGE_REUSE_WARP" in text:
        print(f"already reuse-patched: {path}")
        return
...
"""
    ...
    static bool wrappers_ready = false;
    VPIStatus status = VPI_SUCCESS;
    static int frame_count = 0;
"""
...
"""
    ...
    text = text.replace("VPI_EGLIMAGE_WARP frame=", "VPI_EGLIMAGE_REUSE_WARP frame=", 1)
    path.write_text(text, encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="Patch MMAPI transcode sample to reuse VPI EGLImage wrappers across frames.")
    ...
    print(f"patched EGLImage-wrapper reuse transcode: {args.sample_dir}")
    return 0
