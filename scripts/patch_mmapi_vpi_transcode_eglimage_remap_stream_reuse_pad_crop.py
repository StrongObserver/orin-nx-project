...
def patch_stream_reuse(path: Path) -> None:
    ...
    if "VPI_EGLIMAGE_REMAP_STREAM_REUSE_PAD_CROP" in text:
        print(f"already Remap stream-reuse pad/crop patched: {path}")
        return
...
"""
    ...
    text = text.replace(
        "VPI_EGLIMAGE_REMAP_PAD_CROP frame=",
        "VPI_EGLIMAGE_REMAP_STREAM_REUSE_PAD_CROP frame=",
        1,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Patch Remap pad/crop MMAPI sample to reuse only the VPI stream while recreating EGLImage wrappers per frame."
    )
    parser.add_argument("--sample-dir", type=Path, required=True)
    args = parser.parse_args()
    path = args.sample_dir / "multivideo_transcode_main.cpp"
    patch_remap_pad_crop_main(path)
    patch_stream_reuse(path)
    patch_makefile(args.sample_dir / "Makefile")
    print(f"patched Remap stream-reuse pad/crop transcode: {args.sample_dir}")
    return 0
