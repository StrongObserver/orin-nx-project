...
vpi_warp_egl_images(EGLImageKHR input_egl, EGLImageKHR output_egl)
{
...

    {
        ...
        {
            cerr << "VPI_EGLIMAGE_WARP frame=" << frame_count << " elapsed_ms=" << elapsed_ms
...
vpi_fail:
    ...
    cerr << "VPI EGLImage warp failed status=" << status << " msg=" << msg << endl;
...
def patch_main(path: Path) -> None:
    ...
    if "VPI_EGLIMAGE_WARP" not in text:
        text = text.replace(
...
"""
    insert_replacement = """            ret = NvBufSurfaceFromFd(ctx->dmabuff_fd[v4l2_buf.index], (void**)(&nvbuf_surf));
            ...
            {
                abort(ctx);
                cerr << "VPI EGLImage scratch buffers are not allocated" << endl;
                break;
            }
            ...
            {
                abort(ctx);
                cerr << "Error while transforming transcode DMABUF to VPI input scratch" << endl;
                break;
            }
            ...
            {
                abort(ctx);
                cerr << "Error while calling NvBufSurfaceFromFd for VPI input scratch" << endl;
                break;
            }
            ...
            if (input_scratch_surf->surfaceList[0].mappedAddr.eglImage == NULL)
            {
                ...
                {
                    abort(ctx);
                    cerr << "Unable to map VPI input scratch fd to EGLImage" << endl;
                    break;
                }
            }
            ...
            {
                abort(ctx);
                cerr << "Error while VPI warp on EGLImage scratch buffers" << endl;
                break;
            }
            ...
            static int egl_stage_frame = 0;
            static double egl_stage_total_ms = 0.0;
            egl_stage_frame++;
            double input_transform_ms = std::chrono::duration<double, std::milli>(egl_stage_t1 - egl_stage_t0).count();
            double wrapper_call_ms = std::chrono::duration<double, std::milli>(egl_stage_t3 - egl_stage_t2).count();
            double output_transform_ms = std::chrono::duration<double, std::milli>(egl_stage_t5 - egl_stage_t4).count();
            double total_stage_ms = std::chrono::duration<double, std::milli>(egl_stage_t5 - egl_stage_t0).count();
            egl_stage_total_ms += total_stage_ms;
            if (egl_stage_frame <= 5 || egl_stage_frame % 100 == 0)
            {
                cerr << "EGL_STAGE_TIMING frame=" << egl_stage_frame
                     ...
                     << " avg_total_stage_ms=" << (egl_stage_total_ms / egl_stage_frame)
                     << endl;
            }

            if (ctx->enc_output_memory_type == V4L2_MEMORY_DMABUF)
"""
    if "VPI EGLImage scratch buffers are not allocated" not in text:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch MMAPI transcode sample to run VPI PerspectiveWarp through official EGLImage wrappers instead of manual CUDA pitch pointers.")
    parser.add_argument("--sample-dir", type=Path, required=True)
    args = parser.parse_args()
    patch_main(args.sample_dir / "multivideo_transcode_main.cpp")
    patch_makefile(args.sample_dir / "Makefile")
    print(f"patched EGLImage-wrapper transcode: {args.sample_dir}")
    return 0
